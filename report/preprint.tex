...
the present one, the hidden computation is held invariant --- exactly on the proven channel,
near-exactly end to end --- while past memories are consolidated
...
the best offline-night schedule ($90.7\pm0.4\%$) at $2.4\times$ the night's replay samples
(and reaching $91.8\pm0.3\%$ within $1.2\times$ of that budget with eight-sample
...
unmasked replay and offline nights degrade sharply at the same sizes; the headline orderings
replicate on a held-out split never used for selection. Rotation's i.i.d.\ price is governed by the substrate's activity fraction: at the $5\%$
...
dataset-independent ratio target in place of per-dataset tuning --- trades at most $1.4$ sequential
...
suppression mask off (\kwta{} only), so test order and batch size cannot affect it. All
...
isolated) and leaves the prediction rate ($0.30\%$; $0.000\%$ isolated), the margin-violation
rate and the accuracy unchanged ($92.2$ vs.\ $92.0$, one seed): the residual drift is the
unenforced margin, not the ordering. (An earlier logged check that reported zero drift
...
The night costs $R_s = 480\times256 \approx 1.2\times10^5$ samples. The headline configuration
(one $16$-sample micro-batch per waking batch, $92.7\pm0.3$) costs
$18{,}760\times16\approx3.0\times10^5$ samples ($2.4\times$ the night). Two schedules meet
the night's budget within $1.2\times$: eight-sample micro-batches at every waking batch
($1.5\times10^5$, $91.8\pm0.3$) and $16$-sample micro-batches at every second one
($1.5\times10^5$, $91.7\pm1.0$ sequential, $93.5\pm0.4$ static); pressure-triggered bursts
with batch $64$ cost $1.8\times10^5$ ($1.4\times$) for $89.1\pm1.1$. Accuracy is bought by
...
this paper was chosen on the official test set. All configurations were frozen before the
following check, in which a stratified tenth of the training set is held out as the
evaluation set and the models never train on it (during development those samples were
training data only; no selection ever looked at them). Over six seeds, rotation reaches
$91.6\pm0.3$ against the same-substrate night's $89.0\pm3.5$ (paired margin $+2.6$
$[+0.2,+5.2]$), BP+ER's $88.8\pm0.3$ ($+2.8$), unmasked ER's $85.4\pm3.5$ ($+6.3$), the
silent mask's $88.0\pm0.7$ and $19.4$ without a buffer; held-out numbers sit $0.9$ points
...
samples ($92.3\pm0.1\to91.8\pm0.3$), while at batch $16$ unmasked replay without rotation
sits at $87.0\pm1.1$, isolation without rotation at $88.8\pm0.8$, and the offline night
falls from $90.9$ to $87.3\pm3.2$ when its batches shrink to $16$. The fourth cell of the
square --- rotation with unmasked replay --- reaches $92.1\pm0.5$ at batch $16$, $91.2\pm0.4$ at
$8$ and $89.5\pm0.9$ at $4$, where the masked system itself turns bimodal ($63.4\pm46.4$): the
two factors are strongly sub-additive, rotation is the larger single factor ($+5.1$ alone,
$+3.9$ on top of isolation), and isolation's marginal accuracy is $0.6$--$0.7$ points at
batches $8$--$16$, within seed noise, with no static-side difference ($94.7$ either way).
What isolation buys at this configuration is the guarantee: with it no replay update can
touch the units the present input uses; without it every micro-batch perturbs the live
coalition, and the outcome rests on the rotation's tolerance of that perturbation. The
earlier ``$39\times$'' cost of inference-time consolidation was an artefact of counting
$256$-sample batches.
...
The full system is insensitive down to $8$ samples; rotation with unmasked replay (dashed)
trails it by $0.6$--$0.7$ at $8$--$16$ and holds at $4$, where the masked system turns
bimodal; without rotation, unmasked and isolated replay and the offline night all degrade. \textbf{Right
...
perturbing the live computation, a double loss. This reversed-mask control thus loses fifteen points ($20$, with high seed variance
$70.3\pm10.1$, at the historical $5\%$ fraction); it is one instantiation of the
...
floor). Homeostatic triggering consistently outperforms the tested surprise trigger during
...
$88.8$ for the same mask on natural silence alone), rotation and isolation together $+5.7$
with a $3.7$-fold smaller seed deviation ($92.7\pm0.3$ vs.\ $87.0\pm1.1$), isolation on top
of rotation $+0.6$ ($92.1\pm0.5$ for rotation with unmasked replay), and replay is worth
everything ($19.6$ is the forgetting floor). Replay confined to the readout row, hidden
layers frozen for replay, is at chance ($27.3\pm15.1$) and costs $12$ static points: the
effect is hidden-layer consolidation entirely. A random suppression of matched size in place
of the refractory rule recovers most of rotation's gain ($90.8\pm1.3$) but trails it by $1.9$
points on both axes with $1.8\times$ the forgetting ($F{=}9.4$ vs.\ $5.3$): alternating the
coalition carries most of the effect, resting the recent winners the rest. The static column
identifies who pays: rotation is the only statically costly component ($94.7$ always-on
against $95.9$--$96.1$ without it, and $94.7$ with rotation and no isolation), a price of
$\approx\!1.2$ points at the record fraction. The unmasked control is not handicapped by the replay step ($3\eta$ against a waking step
of $\eta/16$): sweeping the replay gain over $\{1/16,1/4,1,3,10\}$ moves masked replay
$71.9\to88.2\to91.8\to92.7\to90.8$ and unmasked replay
$63.8\to83.3\to86.2\to87.0\to85.5$ (sequential, three seeds) --- both peak at the
default and the unmasked control never closes the gap. The gates that were built to refund a larger price at the
...
and either remedy --- letting the moments track every gradient (surrendering that temporal confinement)
...
drive-over-weight ratio: all referenced to the learner's own current state, none absolute. A
...
\emph{any} configuration we tested, isolation-free included --- a margin that did not
replicate cleanly on the held-out split (Appendix~\ref{app:val}) and that we therefore treat
as provisional. On CIFAR-100 the controller lifts the system
...
exact: under the tuned fixed decay without skips the static axis dies at four hidden layers ($9.7\pm0.7\%$,
...
system returns to the two-layer level on \emph{both} axes: $91.5\pm0.4$ sequential,
$96.2\pm0.3$ static --- against the two-layer controller cell's $91.7/96.1$, and still $1.2$
sequential points below the two-layer fixed-decay default --- with $96.3\pm0.2$ at depth
four the best i.i.d.\ accuracy of the project. The bypass alone also rescues the fixed decay ($91.3/93.6$ at depth five), consistent with
...
re-tuning, not that no fixed value would serve. A BP+ER reference at
...
margin; all configurations were frozen before the held-out check of Section~\ref{sec:results}
and Appendix~\ref{app:val} (a tenth of the training data: training data only during
development, never consulted by any selection, never trained on in the check), which
replicates the headline orderings on one split of MNIST and CIFAR-10 (Spearman $\rho=0.97$,
mean gap $0.9$ points on MNIST). The protocol is five epochs per task, not a single pass
...
included --- keeps a $1.7$-point lead on feature-based CIFAR with both sides at their best
...
stream is using. Its accuracy value is large without rotation ($-5.7$ points) and small with
it ($-0.6$): once the coalition alternates, the live units tolerate unmasked replay, and what
isolation adds is the guarantee that they need not. With isolated replay, refractory
...
$2.4\times$ the night's replay samples (within a point of it at $1.2\times$), pays about
a point on i.i.d.\ streams, gains
...
selection margin. As a check on the \emph{orderings}, which carry the claims, every
configuration was frozen and the headline configurations were re-run with a tenth of the
training set held out as the evaluation set (stratified, fixed split; training data during
development, never consulted by any selection, never trained on here) and the remaining nine
tenths as the stream, six seeds each (Table~\ref{tab:val}; the CIFAR-10 check is in
Table~\ref{tab:valcifar}). The held-out numbers sit $0.9$ points below the test
numbers on average (a smaller training set and a distribution-matched evaluation set both
contribute), the sequential-axis ranking correlates at Spearman $\rho=0.97$, and the headline
MNIST orderings survive (a single fixed split): always-on rotation and
the bout gate lead ($91.6\pm0.3$/$91.8\pm0.3$ vs.\ the night's $89.0\pm3.5$, BP+ER's
$88.8\pm0.3$ and unmasked ER's $85.4\pm3.5$; paired bootstrap margins $+2.6$ $[+0.2,+5.2]$,
$+2.8$ $[+2.5,+3.2]$ and $+6.3$ $[+3.8,+9.1]$ over six seed pairs); the silent mask trails
rotation; the five-hidden-layer skip system holds the two-layer level on both axes
($91.0$/$95.8$). One claim does not replicate cleanly: the adaptive decay's static-side
margin ($96.1$ vs.\ $94.7$ on test) becomes $93.5\pm4.3$ vs.\ $94.1\pm0.2$ on the held-out
split, where one of six seeds collapsed --- we report it as unconfirmed. On CIFAR-10
(Table~\ref{tab:valcifar}, three seeds) every ordering replicates: raw
$28.9>25.7>24.8>15.4\approx16.3$ for rotation, night, BP+ER, unmasked ER and no buffer;
features $42.3>34.0\gg20.5$ with BP+ER at $42.8$ ($0.5$ ahead, against $1.7$ on test) and
the decomposition chain $43.7/43.5/40.4$.

\begin{table}[h]
\centering\small
\caption{\textbf{Held-out replication.} Sequential / static accuracy (\%) on the official
test set (all available seeds, three to six) and on a held-out tenth of the training set
never used for training (six seeds).}
\label{tab:val}
\begin{tabular}{lcccc}
\toprule
