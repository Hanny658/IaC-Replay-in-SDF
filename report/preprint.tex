...
that the residual advantage of offline nights is confined to very small episodic buffers. We
...
\pending{split CIFAR-10 with a frozen V1-like random-patch feature front-end (1024-D, linear
probe $\approx49\%$) is running; all cifar-feature numbers below are placeholders.}
...
\pending{a progress-keyed variant --- rotate while $\bar e_f \ge \gamma\, e_0$ (error still a
$\gamma$ fraction of its chance level $e_0$) OR condition \eqref{eq:nov} --- is running; it is
designed to keep rotation on for never-converging streams where rotation is beneficial.}
...
\node[align=left, anchor=west] at (2.7, 2.2)
  {\tikz\node[awake]{}; wake-active \quad \tikz\node[asleep]{}; silent};
\node[align=left, anchor=west] at (2.7, 1.65)
  {\tikz\node[refr]{}; refractory (forced rest)};
\node[align=left, anchor=west] at (2.7, 1.1)
  {\textcolor{green!55!black}{dashed}: replay may write\\ (pre silent $\lor$ post asleep, Eq.~\ref{eq:mask})};
\node[align=left, anchor=west] at (2.7, 0.45)
...
\draw[->, dashed] (burst.west) to[bend left=40] node[left]{discharge} (press.west);
\draw[->] (wake.east) -| (nov.north);
...
--- forced rotation acts as a regulariser in the underfit regime --- and the novelty gate
correctly stays open ($\ge95\%$) there under the progress-keyed variant.
\pending{final 3-seed numbers for the progress-keyed gate on all four axes (running).}
...
\pending{the same grid on the V1-like feature front-end; first seeds show the ordering holds for
the local variants (refractory $37$, unmasked $31$, night $31$) but BP+ER strengthens markedly
($43$) --- to be reported honestly as the regime where the local learner loses the lead.}

\subsection{Small buffers: what the night still buys}
At $K{=}200$ the night keeps an edge (Figure~\ref{fig:k200}). Replay diversity is the main lever
during wake: noise $\sigma{=}0.2$ on replayed samples plus pressure bursts lifts local sleep from
$81.8\pm2.9$ to $84.3\pm1.0$ (forgetting $0.08$), but the night reaches $86.9$. Mechanistically:
(i) \emph{unmasked} waking replay is far worse ($72.9$) --- isolation is a benefit, not the cost;
(ii) the same noise \emph{hurts} the offline night ($85.8\to78.1$) --- diversity is specifically a
wake-side lever; (iii) a night added on top of noisy local sleep recovers little.
The remaining gap is the offline window itself: with $200$ samples carrying ten classes, every
waking batch between replays erodes the buffer's trace, and only interference-free consolidation
locks it. Local sleep replaces the night only when the buffer is rich enough ($K\gtrsim10^3$) to
sustain continuous re-writing. \pending{third seeds for three of the seven bars.}
...
rotation gates. \pending{sleep-anchored structural plasticity (prune/regrow after consolidation
windows vs.\ a clock schedule; Tononi--Cirelli down-selection) is running --- subsection to be
added.}
...
learner on feature-based CIFAR (pending); the novelty gate needs its progress-keyed extension for
never-converging streams; and the novelty claims rest on our own literature search (August 2026).

% ---------------------------------------------------------------- 7
\section{Future Work}
\label{sec:future}

(i) The progress-keyed rotation gate unifying converging and non-converging regimes
(\pending{running}); (ii) sleep-anchored synaptic down-selection --- pruning immediately after
consolidation windows so that replay-potentiated synapses are protected and KP-decayed ones are
removed (\pending{running}); (iii) feature front-ends and convolutional stacks to test the
mechanism beyond MLPs; (iv) closing the small-buffer gap with wake-compatible interference
shields; (v) a throughput theory of the isolated channel predicting the required replay cadence
from $\rho_\ell$, code overlap and the interference rate; (vi) the rotation--spiking calibration
interaction.
