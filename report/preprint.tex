...
gate numbers in this subsection use masked Adam; the gate's adverse interaction with the
stateless optimiser is dissected in Section~\ref{sec:ablation}.)
...
full: exact isolation $+$ rotation $+$ progress gate & $89.1\pm0.3$ & $95.0\pm0.6$ \\
$-$ gate (rotation always on) & $\mathbf{92.0\pm0.7}$ & $93.4\pm0.2$ \\
...
the price in full ($95.0$, at the $95.1$ ceiling). The honest headline of the table, however,
is that the full conjunction is the best system \emph{nowhere}: against always-on rotation it
loses $2.9$ sequential points ($89.1$ vs.\ $92.0$), and against the rotation-free system it is
sequentially identical ($89.1$ vs.\ $89.0$) yet a full static point worse ($95.0\pm0.6$ vs.\
$96.0\pm0.1$). Under SGD the gated point is weakly dominated: the system has exactly two useful
operating points --- always-on rotation ($92.0/93.4$) for sequential streams, plain silent
isolation ($89.0/96.0$) for stationary ones --- and the gate manufactures no third. The
mechanism is the half-cadence effect again: the stateless gain requires rotation on
\emph{every} batch, and within each task the gate closes as soon as the task is mastered ---
precisely the batches on which continuous rotation earns its $+3.0$. Under Adam the same gate
is genuinely adaptive: it captures most of rotation's sequential value ($90.4$ gated vs.\
$87.8$ silent) at a modest static cost ($94.3$ vs.\ $95.0\pm0.3$ for silent
isolation), an undominated compromise. The gate's value is thus a property of the optimiser it wraps,
not of the gate alone.
...
gate keeps a $0.7$-point static residue under Adam and is weakly dominated by rotation-free
isolation under the stateless optimiser (Section~\ref{sec:ablation}); and the novelty claims rest on our own literature
...
itself); (ii) a rotation gate compatible with the stateless optimiser --- since the SGD gain requires
rotation on every batch, a useful gate needs block-wise switching with a minimum on-duration
