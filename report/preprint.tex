...
resulting system reaches $91.3\pm0.3\%$ ($92.0\pm0.6\%$ with a stateless optimiser) and no
offline phase, exceeding the best offline-night schedule ($90.7\pm0.4\%$), at a replay-sample
cost of $\approx\!1.2\times$ the night's; reversing the protected direction (guarding past
memories while learning the present, as in null-space methods) collapses to $81.3\pm2.9\%$ under
identical machinery; masked
...
points) is eliminated, under masked Adam, by the relative-novelty gate on learnable streams ($95.0\%$ vs.\ $95.0\%$
without rotation); under a stateless optimiser the same gate must instead be committed to
sleep-bout-like episodes with a minimum on-duration, which then weakly dominate always-on
rotation ($92.0/94.2$ vs.\ $92.0/93.3$ over six seeds). The substrate's $5\%$ activity
fraction, an early inherited choice, turns out to tax both regimes: relaxing it to
$10$--$15\%$ improves both axes at once ($92.7/94.7$ on split-MNIST) and shrinks
at source the price the gate exists to refund. The price reverses into a $+2.5$ to $+6.5$ point \emph{gain} on harder data
...
$64$) reaches $91.3\pm0.3\%$ with no offline phase, above the best offline night
($90.7\pm0.4\%$) and far above unmasked interleaved ER at the same budget ($86.1$--$86.9\%$);
the stateless-SGD variant of Section~\ref{sec:direction} reaches $\mathbf{92.0\pm0.6\%}$.
...
$87.8\pm2.1$, and the offline night to $81.0\pm1.6$ at batch $16$. Masking the replay weight update is what makes replay affordable: the earlier
...
applied unmasked. With everything else identical, mirror isolation reaches only $81.3\pm2.9\%$
--- below even unmasked interleaved ER ($86.9\pm1.2$) and far below our direction
($90.8\pm0.1$): constraining wake learning impairs the new task while the unmasked replay keeps
perturbing the live computation, a double loss. The protected direction is thus not a framing
choice but a $9.5$-point empirical effect. Second, replacing masked Adam with \emph{stateless}
heavy-ball SGD inside the same isolation raises the ceiling: $\mathbf{92.0\pm0.6\%}$ sequential
and $93.3\pm0.2\%$ static (six seeds; vs.\ $90.8/92.0$ for masked Adam; unmasked SGD control
$84.1\pm4.1$). Carrying no state at all is simpler and better still --- and closer to biology;
the ablation in Section~\ref{sec:ablation} traces masked Adam's deficit to the confinement of
its moments, which the exactness guarantee demands but accuracy, it turns out, does not. Third, \emph{soft} rotation (halving,
rather than silencing, recent winners) loses on both axes ($88.4\pm0.7$ sequential,
$86.0\pm1.3$ static): all-or-none OFF periods beat turned-down gain, echoing the biological
finding that tonic firing-rate reduction does not discharge local sleep pressure while ON/OFF
alternation does \citep{driessen2026}. Table~\ref{tab:ablation} in
Section~\ref{sec:ablation} collects these controls together with the remaining component
knockouts and the substrate ladder.
...
$35.6\pm1.3$ vs.\ $31.7\pm3.6$), and BP+ER reaches $42.6\pm0.5$ --- the reverse of the
raw-pixel ordering. In our first report this read as a $5.6$-point regime boundary; a
two-sided fairness pass revises it. The stateless optimiser transfers to the feature regime
($40.6\pm1.0$ sequential at $\eta{=}0.01$, six seeds; static $38.6\pm1.4$), while BP+ER at
its own best learning rate gains little ($43.6\pm1.1$ at $3{\cdot}10^{-4}$, six seeds). With
both sides tuned the honest gap is $3.0$ points, not $5.6$: most of the boundary was the local
learner's optimiser, not its learning rule. On raw pixels the stateless gain does not appear
($27.2\pm1.4$ vs.\ $27.1\pm1.7$ for masked Adam): statelessness pays in the converged and
feature regimes, not in the underfit one.

The residual $2.7$ points then decompose entirely into the \emph{substrate}, not the learning
rule. Transplanting the local network's ingredients into the BP net one at a time (each cell at
its better of two learning rates): its width \emph{helps} BP+ER ($44.8\pm0.9$ for a dense
$512$--$256$ net), its $30\%$ distance-dependent wiring --- drawn by the same generator on the
same sheets --- costs $1.2$ ($43.6\pm1.0$), and its $5\%$ \kwta{} costs another $3.9$ points
($39.7\pm0.9$), leaving backprop-with-replay \emph{at or below} the local learner on the
identical substrate ($40.6\pm1.0$; all cells six seeds). What separates the two systems on feature inputs is dense
activation, not backpropagation's credit assignment: on the substrate this project commits to,
the local rule gives nothing away. The advantage over \emph{other local schedules} (nights,
unmasked ER) remains robust across all three input regimes.
...
\subsection{Energy}
Converted to spiking inference ($T_s{=}8$), the current default system (bout-gated rotation,
stateless SGD; rate accuracy $94.5\%$ on the measured seed) runs at $93.5\%$ and
$118$\,nJ/sample vs.\ $2461$\,nJ for dense rate inference ($20.8\times$) and $516$\,nJ for
event-driven rate inference ($4.4\times$), improving further with the time budget ($94.0\%$
at $T_s{=}16$, $239$\,nJ). The calibration interaction we reported for the earlier Adam stack
...
degrades beyond $T_s{=}8$ ($94.5\to91.2$ at $T_s{=}32$) --- consistent with rotation shaping
sparser codes (final hidden dimensionality $12.7$ vs.\ $28.8$; $60$ vs.\ $91$ spikes per
sample at $T_s{=}8$).
...
rotation gates; pruning anchored to sparse replay bursts (above); soft (gain-reduction) rotation
...
optimiser, and the progress-keyed gate \eqref{eq:prog}. This section removes or replaces one
ingredient at a time, at two levels: the consolidation mechanism (Table~\ref{tab:ablation}) and
the substrate beneath it (Table~\ref{tab:substrate}). Every mechanism cell shares the same
substrate ($512$--$256$ hidden, $5\%$ \kwta{}, $30\%$ distance-dependent connectivity), the same
buffer ($K{=}1000$, random writing) and the same replay budget (waking batch $16$, replay batch
$16$ after every waking batch), three seeds each.

\begin{table}[t]
\centering
\caption{\textbf{Component ablation of the consolidation mechanism.} Sequential $=$ final
accuracy on split-MNIST; static $=$ i.i.d.\ MNIST (the axis that must not degrade). All rows use
the $512$--$256$/$5\%$ substrate except the backprop references (dense $256$--$128$). Dashes:
combination not applicable. Bold: column best. The bout-gate and always-on rows carry six
seeds; all others three.}
\label{tab:ablation}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
...
full: exact isolation $+$ rotation $+$ per-batch progress gate & $89.1\pm0.3$ & $95.0\pm0.6$ \\
\quad the same gate committed to bouts (min on-duration $2048$) & $92.0\pm0.4$ & $94.2\pm0.4$ \\
$-$ gate (rotation always on) & $92.0\pm0.6$ & $93.3\pm0.2$ \\
$+$ two-timescale anchor ($\lambda{=}\mu{=}3{\cdot}10^{-4}$), rotation always on & $\mathbf{92.4\pm0.2}$ & $93.3\pm0.2$ \\
$-$ rotation (pre-silent isolation only) & $89.0\pm0.5$ & $\mathbf{96.0\pm0.1}$ \\
$-$ isolation (unmasked interleaved replay) & $84.1\pm4.1$ & $95.7\pm0.1$ \\
$-$ replay (no episodic buffer; Adam) & $19.5\pm0.0$ & $95.1\pm0.1$ \\
\midrule
\multicolumn{3}{@{}l}{\emph{Optimiser state under the mask (rotation always on, Adam)}}\\
moments confined to the mask & $90.8\pm0.1$ & $92.0\pm0.3$ \\
moments advancing outside the mask & $91.9\pm0.4$ & $92.8\pm0.4$ \\
\midrule
\multicolumn{3}{@{}l}{\emph{Replacing the construction (Adam)}}\\
offline night instead of concurrent replay, same substrate & $84.1\pm1.3$ & --- \\
\quad the same night on its preferred narrow substrate & $90.7\pm0.4$ & --- \\
mirror direction: protect the past (Section~\ref{sec:direction}) & $81.3\pm2.9$ & $92.1\pm0.1$ \\
soft rotation: gain $0.5$ instead of $0$ & $88.4\pm0.7$ & $86.0\pm1.3$ \\
\midrule
\multicolumn{3}{@{}l}{\emph{External references}}\\
unmasked ER at the same budget (Adam) & $86.9\pm1.2$ & $94.8\pm0.2$ \\
...
sequential axis under the stateless optimiser: rotation is worth $+3.0$ points ($92.0$ vs.\
$89.0$ for the pre-silent channel alone), isolation $+7.9$ with a six-fold variance reduction
($92.0\pm0.6$ vs.\ $84.1\pm4.1$), and replay is worth everything ($19.5$ is the forgetting
floor). The static column identifies who pays: rotation is the only statically costly
component ($93.3$ always-on against $96.0$ for isolation without rotation --- the highest
static cell in the table, above even the no-buffer baseline), and the progress gate refunds
the price in full ($95.0$, at the $95.1$ ceiling). The \emph{per-batch} gated conjunction,
however, is the best system nowhere: against always-on rotation it loses $2.9$ sequential
points ($89.1$ vs.\ $92.0$), and against the rotation-free system it is sequentially identical
($89.1$ vs.\ $89.0$) yet a full static point worse ($95.0\pm0.6$ vs.\ $96.0\pm0.1$) --- it
manufactures no operating point that always-on rotation and plain silent isolation do not
already offer. Under Adam the same per-batch gate \emph{is} genuinely adaptive ($90.4$ gated
vs.\ $87.8$ silent sequentially, at a modest static cost, $94.3$ vs.\ $95.0\pm0.3$), so the
failure is specific to the stateless optimiser, whose rotation gain the gate's flicker
destroys: the trigger is open on only $12\%$ of batches, and within each task it closes the
moment the task is mastered --- precisely the batches on which continuous rotation earns its
$+3.0$.

Committing the gate to \emph{episodes} repairs it. In the bout-committed variant, every firing
of the same trigger re-arms a rotation episode with a minimum on-duration of $M$ waking batches
--- the consolidated-bout structure of the sleep--wake flip-flop switch \citep{saper2005}, in
which state flicker is the pathology, not the mechanism. The sparse trigger is thereby
converted into long contiguous episodes whose coverage is regime-asymmetric: at $M{=}2048$,
bouts chain into near-permanent rotation on the sequential stream ($94\%$ of batches) yet cover
only $63\%$ of the static one. The result --- $92.0\pm0.4$ sequential at $94.2\pm0.4$ static over six
seeds --- matches always-on rotation exactly on the sequential axis ($92.0\pm0.6$) while
refunding $0.9$ of its static price ($93.3\pm0.2$): a clean weak domination that retires
always-on rotation as an operating point. Bout length is a proper knob: $M{=}512$ (duties
...
The middle block tests the exactness requirement itself and yields the sharpest surprise of
the table. Propositions~\ref{prop:pre}--\ref{prop:post} require Adam's moments to advance only
inside the mask: state advancing outside re-injects the replay gradient into awake synapses
through the next unmasked waking step, and output-neutrality is lost. Empirically, however,
the leak \emph{helps} --- $91.9\pm0.4$ vs.\ $90.8\pm0.1$ sequential, $92.8\pm0.4$ vs.\
$92.0\pm0.3$ static. Exact state confinement, not isolation, is what held masked Adam below
the stateless optimiser: moments frozen outside each batch's mask go stale across a unit's
on/off cycles \citep{bricken2023}, and either remedy --- letting the state track every
gradient (surrendering the guarantee) or carrying no state at all --- recovers the same
$\approx\!1$ point. Statelessness is the only choice that keeps the guarantee and the
accuracy at once: the exactness of Propositions~\ref{prop:pre}--\ref{prop:post} costs about
one point when bought from Adam, and nothing when bought from heavy-ball SGD.

Two further notes complete the picture. The stateless gain also vanishes at half cadence:
SGD with replay batch $64$ every other waking batch reaches $91.3\pm0.3$, identical to masked
Adam on the same schedule --- statelessness pays only while consolidation runs continuously.
And the replacement block collects, in one place, that every structural alternative loses on
the sequential axis: the offline night on this substrate ($84.1$), the protected-past
direction ($81.3$), softened rotation ($88.4$), unmasked ER ($86.9$), with the strongest
competitor --- the night on its preferred narrow substrate ($90.7$) --- still below the
always-on operating point.
...
The OFF-refractory null located the bout gate's residual static price ($94.2$ vs.\ $96.0$
rotation-free) in the rotation of settled coalitions itself. Two mechanisms attack that
diagnosis directly; one is a clean negative, one a modest positive.
...
a \emph{sequential stabiliser}: $92.4\pm0.2$, the best sequential cell in
Table~\ref{tab:ablation} with the seed variance halved, at static parity --- weakly dominating
plain always-on rotation. Anchor and bout gate compose sub-additively
($92.3\pm0.3/93.8\pm0.5$, between the parents). Figure~\ref{fig:frontier} draws the
resulting operating frontier.

\begin{figure}[t]
\centering
\includegraphics[width=.6\linewidth]{figs/fig_frontier.pdf}
\caption{\textbf{The operating frontier} under the stateless optimiser: split-MNIST
sequential accuracy against i.i.d.\ static accuracy (six seeds on the bout-gate, always-on and
dial points, three elsewhere). Blue: non-dominated configurations at the historical $5\%$
activity fraction; grey: dominated ones; orange diamonds: the same machinery with the
\kwta{} fraction relaxed to $10$--$15\%$ (Section~\ref{sec:dial}), which shifts the whole
frontier outward. Within a fraction, no single configuration wins both axes.}
\label{fig:frontier}
\end{figure}

\subsection{The sparsity dial}
\label{sec:dial}
The substrate's $5\%$ \kwta{} fraction was fixed in an early phase and inherited by every
experiment above; the decomposition just given makes it suspect --- if $5\%$ \kwta{} taxes
BP by four points on features, it presumably taxes our own learner too. It does, on both
regimes, and the tax can be bought back almost for free. On feature CIFAR-10, relaxing the
fraction improves \emph{both} axes monotonically: sequential $40.6\to42.8\pm0.3$ and static
$38.6\to45.6\pm0.5$ from $5$ to $25\%$, with forgetting slightly \emph{reduced}; the BP
control traces the same curve ($39.7\to41.2\to43.6$), with the local learner
$\approx\!1.1$ points ahead at every matched level. On MNIST the sequential optimum sits at
$10\%$: always-on rotation reaches $\mathbf{92.7\pm0.3}$ sequential at $94.7\pm0.3$ static
(six seeds) --- dominating every $5\%$ cell of Table~\ref{tab:ablation}, bout gate and
...
and spiking inference stays input-dominated ($94.0\%$ at $120$\,nJ, $T_s{=}8$, at $15\%$;
monotone in $T_s$ as before). We keep $5\%$ as the substrate of record for
Tables~\ref{tab:ablation}--\ref{tab:substrate} --- every mechanism comparison above is
internally matched, and the orderings we spot-checked at $10$--$15\%$ are preserved --- but
the dial, not the gate, is the cheapest cure for rotation's static price
(Figure~\ref{fig:frontier}, orange).

\subsection{The price of the substrate}
...
All results are at MLP scale on MNIST/CIFAR variants with three seeds (six on the headline
rows); BP+ER keeps a $3.0$-point lead on feature-based CIFAR with both sides at their best
learning rates --- a lead the substrate decomposition attributes to dense activation rather
than to backpropagation itself; the mechanism tables use the historical $5\%$ activity
fraction, which the sparsity dial (Section~\ref{sec:dial}) shows is not the optimum; the progress
gate keeps a $0.7$-point static residue under Adam, must be committed to minimum-duration
bouts under the stateless optimiser, and even then leaves a $1.8$-point static gap to
rotation-free isolation (Section~\ref{sec:ablation}); and the novelty claims rest on our own literature
...
(i) Re-baselining the full mechanism suite on the $10\%$ substrate the sparsity dial
identifies (the tables above use the historical $5\%$); (ii) the bout gate's residual static price ($94.2$ vs.\ $96.0$ rotation-free): the OFF-side
...
reverses between the Adam and the stateless stack.
