...
fraction the project inherited, novelty gates (masked Adam) or sleep-bout-committed gates
(stateless SGD) were needed to refund it, whereas at the $10\%$ optimum the price shrinks to
about a point and both gates become neutral --- the sparsity dial, not the gate, is the cure.
...
that defeat every fixed decay; with locally learned ResNet-style skip synapses the
...
adaptation signal \citep{hasselmo2006}). Monitoring streaming error with exponentially weighted
...
$90.9\pm3.2$, wildly unstable, on ours) and far above unmasked interleaved ER at the same
budget ($89.7\pm1.1$). Halving the cadence with doubled replay batches costs $1.3$
($91.4\pm0.3$). Adding a night on top of continuous local sleep yields nothing
...
(record substrate, stateless SGD). Masked refractory replay is insensitive down to $8$
...
Second, replacing masked Adam with \emph{stateless} heavy-ball SGD inside the same isolation
...
Carrying no state at all is simpler and better --- and closer to biology; the ablation in
...
events), and surprise triggering fails outright: at the record fraction the converged error
never crosses the surprise threshold, so \emph{zero} replay events fire ($19.0$, the
forgetting floor). The waking trigger must be homeostatic --- the opposite of the night's
...
rotation-free) and the CIFAR static effect becomes statistically neutral ($27.6\pm0.4$ vs.\
$29.5\pm2.8$). Under masked Adam, a relative-novelty/progress gate
...
Appendix~\ref{app:gates}. Under the stateless optimiser the gate had to be committed to bouts
...
raw-pixel ordering. In our first report (at the historical $5\%$ fraction, local learner at
$36.8$) this read as a $4.7$-point regime boundary; a two-sided fairness pass plus the
sparsity dial revise it to $1.7$ ($41.5\pm1.1$ local vs.\ $43.2\pm1.0$ BP+ER): most of the
...
The residual gap then decomposes entirely into the \emph{substrate}, not the learning rule.
...
the substrate this project commits to, the local rule gives nothing away. The advantage over
...
the default system (always-on rotation, stateless SGD, record fraction; rate accuracy
...
has \emph{reversed sign} under the stateless optimiser: the rotation-trained networks are now
...
substrate, the exact isolation mask \eqref{eq:mask}, the refractory rotation, a stateless
...
\midrule
\multicolumn{3}{@{}l}{\emph{Full system and single knockouts (stateless SGD, $\eta=0.02$)}}\\
...
sequential axis under the stateless optimiser: rotation is worth $+3.9$ points ($92.7$ vs.\
$88.8$ for the pre-silent channel alone), isolation $+5.7$ with a four-fold variance reduction
...
gate's flicker destroyed the stateless gain, episode commitment (the sleep--wake flip-flop's
consolidated-bout structure \citep{saper2005}) repaired it, and bout-length and OFF-refractory
...
confinement, not isolation, is what holds masked Adam below the stateless optimiser: moments
frozen outside each batch's mask go stale across a unit's on/off cycles \citep{bricken2023},
and either remedy --- letting the state track every gradient (surrendering the guarantee) or
carrying no state at all --- recovers most of the gap. Statelessness is the only choice that
keeps the guarantee and the accuracy at once.

One budget note completes the block: half the cadence with doubled batches costs $1.3$
($91.4\pm0.3$) --- the stateless system prefers consolidation to run continuously.
...
\caption{\textbf{The operating frontier} at the record fraction under the stateless
optimiser: split-MNIST sequential accuracy against i.i.d.\ static accuracy (headline points
six seeds). Blue: non-dominated configurations; purple: dominated ones; small grey dots: the
historical $5\%$ family the project inherited, entirely inside the new frontier. The two
...
$3$--$5\times$ over the manual rescue with no dataset-specific knob at all (floor-regime
...
numbers are the honest estimates. All results are at MLP scale on MNIST/CIFAR variants with
...
the same-substrate offline night is wildly unstable ($90.9\pm3.2$), a phenomenon we report
...
reverses between the Adam and the stateless stack; (vii) a drive estimate that survives
...
gate numbers in this subsection use masked Adam; under the stateless optimiser the gate must be
...
The stateless comparison invites an objection: SGD's $\eta$ was chosen, Adam's was left at its
...
stateless joint point ($3{\cdot}10^{-3}$ buys $93.6$ static at a three-point sequential cost).
...
contribute), the sequential-axis ranking correlates at Spearman $\rho=0.92$, and every claim
the paper rests on survives: always-on rotation and the bout gate lead ($91.6$/$91.7$ vs.\
