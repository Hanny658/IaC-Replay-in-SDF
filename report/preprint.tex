...
waking hidden code of only $\approx0.3\%$ of samples per replay step elsewhere --- with
...
resulting system reaches $\mathbf{91.6\pm0.3\%}$ (six seeds) with no offline phase, at or above the best offline-night schedules on two held-out splits ($+2.5$ and $+0.3$ over the
best night) and consistently above BP+ER and unmasked local replay, at $2.2\times$ the night's
replay samples (and reaching $91.1\pm0.4\%$ within $1.1\times$ of that budget with eight-sample
micro-batches); in a single pass over the stream the lead widens ($91.8\%$ against $88.7\%$ for
BP+ER and $75.1\%$ for the night); reversing the protected direction (guarding past memories
...
$16$ to $4$, the guarantee, and at two-sample batches none of the severe failures that unmasked replay shows
in half the seeds; $2.6$ points without rotation); main comparisons use frozen configurations
evaluated on a held-out tenth of the training data, with the headline orderings replicated on a
second split, and exploratory results are labelled. Rotation's i.i.d.\ price is governed by the substrate's activity fraction: at the $5\%$
...
suppression mask off (\kwta{} only), so test order and batch size cannot affect it. Configurations were selected on the official test splits, which served as the development set,
and then frozen; every main-text comparison re-trains them with a stratified tenth of the
training data removed from the stream and evaluates on that tenth, which took part in no
selection (a second tenth, drawn with a different seed and sharing $9\%$ of its samples with the
first, replicates the headline rows, Appendix~\ref{app:val}). Results not re-run under this
protocol are labelled development-phase. All
...
configuration under the reported protocol, re-inferring every waking batch after each of its
$16{,}885$ isolated replay updates (under the suppression it was inferred with): the update
altered the top hidden code of $0.33\%$ of the waking samples per step (three seeds,
$0.31$--$0.35$), changed a waking prediction for $0.32\%$ --- almost all of it the plastic
readout, since the same count is $0.001\%$ with the readout isolated (hidden code $0.21\%$; one
seed) --- and violated the margin
condition for $\sim\!10^{-4}$ of the asleep units per step; the mean maximal logit change
per update was $0.02$ ($0.001$ with the readout isolated) on the one-hot scale. On raw CIFAR-10 (one seed, development run) the hidden-code rate is $0.46\%$ while the
prediction rate rises to $3.6\%$, low-margin predictions flipping more easily through the
plastic readout. These rates concern the hidden code of the
...
the post-update awake sets lowers the hidden-code rate from $0.32\%$ to $0.30\%$ ($0.17\%$ with
the readout isolated) and leaves the prediction rate ($0.30\%$; $0.000\%$ isolated), the
margin-violation rate and the accuracy unchanged ($92.2$ vs.\ $92.0$; one seed, development
runs on the full training set, $18{,}760$ updates): the residual drift is the unenforced margin, not the ordering. (An earlier logged check that reported zero drift
...
$16{,}885$ masked updates against the night's $480$ ($35\times$; counts from the reported runs,
whose stream is nine tenths of the training set), and its training wall-clock on one CPU thread is $8.8$ min against $1.4$ for the night, $1.0$ with
...
$K{=}1000$ with the static axis reported alongside, on the official test split (the development set); the comparisons reported are on the
held-out split (Appendix~\ref{app:val}).
...
updates, $1.35\times10^5$, $91.2\pm0.2$ sequential, $93.1\pm0.2$ static); pressure-triggered bursts of $16$ at a third of the events reach $88.6\pm1.6$. Accuracy is bought by
...
Adam, $85.4\pm3.5$ under SGD) and above BP+ER ($88.8\pm0.3$; $+2.8$ $[+2.5,+3.2]$). In a single pass over the stream (one epoch per task, three seeds) the ordering sharpens:
$91.8\pm0.2$ against $88.7\pm0.3$ for BP+ER, $76.0\pm20.1$ for unmasked ER and $75.1\pm3.5$ for
the offline night, whose one night per task is too few (Table~\ref{tab:ablation}).
Halving
...
$75$--$84$), whereas unmasked replay bifurcates --- three seeds at $82$--$85$, one at $58$ and two at chance,
i.e.\ severe failures below $60\%$ in half the seeds (Appendix~\ref{app:isomargin}). In the seeds
and configurations tested, isolation's accuracy value is thus the absence of those failures rather
than a margin; what it buys at every batch size is the guarantee that,
...
destabilise) --- masked replay peaks at the default, unmasked replay plateaus from gain $1$ to $3$,
and the rotation-free unmasked control never closes the gap; the sweep bounds that control, it does not estimate isolation's own
...
the update to the mask adds $0.1$--$0.3$ points at those batch sizes, showed none of the severe failures that unmasked replay shows in half the seeds at batch $2$
(Appendix~\ref{app:isomargin}), and gives
...
samples); exploratory results not re-run under this protocol are labelled development-phase.
Development-set numbers sit $1.1$ points above the held-out ones on average and rank-correlate
with them at Spearman $\rho=0.97$ (a robustness check, not a proof that selection bias is
absent; Appendix~\ref{app:val}). The protocol is five epochs per task; a single pass keeps the lead
(Table~\ref{tab:ablation}, Appendix~\ref{app:probes}). All results are at MLP scale on
...
need not --- and, at two-sample batches where that tolerance runs out, graceful degradation where
the unmasked control failed severely in half the seeds. With isolated replay, refractory
...
stream: it is competitive with the tested offline-night schedules on both held-out splits (above the best
night by $2.5$ on the first, by $0.3$ on the second) and consistently above BP+ER and unmasked
local replay, at $2.2\times$ the night's replay samples (and, on the first split, at $1.1\times$), pays one to two
...
isolated-consolidation loop the two tie (Table~\ref{tab:ablation}). Per-synapse adaptive state
is an asset while every synapse trains on every batch and no longer one once updates become
intermittently mask-confined. The optimiser is thus not a neutral implementation detail: its
advantage on the static substrate does not carry into the consolidating system, where the least
state suffices.
...
(Appendix~\ref{app:probes}). The paired mean at batch $2$ ($+24$ $[-0.2,+48.5]$) describes the
