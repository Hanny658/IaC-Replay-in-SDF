...
rotation ($92.0/94.2$ vs.\ $92.0/93.3$ over six seeds). The price reverses into a $+2.5$ to $+6.5$ point \emph{gain} on harder data
...
its own best learning rate gains little ($43.3\pm1.0$ at $3{\cdot}10^{-4}$). With both sides
tuned the honest gap is $2.7$ points, not $5.6$: most of the boundary was the local learner's
optimiser, not its learning rule. On raw pixels the stateless gain does not appear
...
its better of two learning rates): its width \emph{helps} BP+ER ($44.5\pm0.4$ for a dense
$512$--$256$ net), its $30\%$ distance-dependent wiring --- drawn by the same generator on the
same sheets --- costs only $0.6$ ($43.9\pm0.7$), but its $5\%$ \kwta{} costs $4.4$ points
($39.5\pm0.6$), leaving backprop-with-replay \emph{at or below} the local learner on the
identical substrate ($40.6\pm1.0$). What separates the two systems on feature inputs is dense
...
sequential accuracy against i.i.d.\ static accuracy (six seeds on the bout-gate and always-on
points, three elsewhere). Blue: non-dominated configurations; grey: dominated ones. No single
configuration wins both axes; the bout-committed gate is the sequential-side default, silent
isolation the static-side one.}
\label{fig:frontier}
\end{figure}
...
rows); BP+ER keeps a $2.7$-point lead on feature-based CIFAR with both sides at their best
learning rates --- a lead the substrate decomposition attributes to dense activation rather
than to backpropagation itself; the progress
...
(i) Whether \kwta{}'s feature-regime cost ($4.4$ points for BP and, by implication, for us)
can be bought back --- milder or annealed sparsity, learned thresholds --- without losing the
exact isolation channel that \kwta{} powers; (ii) the bout gate's residual static price ($94.2$ vs.\ $96.0$ rotation-free): the OFF-side
