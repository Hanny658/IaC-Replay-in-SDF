...
\renewcommand{\shorttitle}{Inference as Consolidation}
...
or above the best offline-night schedule on two held-out splits and consistently above
backpropagation with experience replay (BP+ER) and unmasked local replay, at about twice the
night's replay samples; in a single pass over the stream the lead widens ($91.8\%$ against
$88.7\%$ for BP+ER and $75.1\%$ for the night). Rotation carries most of the gain; isolation
adds the invariance guarantee and, at two-sample replay batches, avoids the severe failures
that unmasked replay shows in half the seeds. The ordering transfers to split CIFAR-10, where
a substrate decomposition attributes the remaining gap to BP+ER to dense activation rather
than to credit assignment. Finally, a drive-referenced controller for the rule's synaptic
decay replaces per-dataset tuning with a single ratio target and, with locally learned skip
synapses, holds a five-hidden-layer network at the two-layer level. We prove exactness for
the isolation rule and release all code, checkpoints and diagnostics.
\end{abstract}
...
state, into the exactness construction. Replay \emph{scheduling} has also been studied as a
learned policy \citep{klasson2023}; our contribution there is a specific unit-level
use-pressure trigger and its empirical dissociation from novelty triggering.
...
this protocol are labelled development-phase. All numbers are mean $\pm$ s.d.\ over three
seeds unless marked otherwise. Runs are deterministic at a fixed CPU thread count; because the
...
\paragraph{Baselines.}
(i) Backpropagation with experience replay at equal buffer $K$ (included in every table);
...
(iii) interleaved ER for the local learner; (iv) no buffer.
...
burst of several micro-batches reuses the mask of its waking batch. Throughout, ``during inference'' means between the steps of the stream: the three operations run
...
Table~\ref{tab:ablation}. Selection everywhere was by sequential accuracy at $K{=}1000$ on
the official test split (the development set), with the static axis reported alongside; the
comparisons reported are on the held-out split (Appendix~\ref{app:val}).
...
This is above the best offline night ($89.2\pm1.7\%$ on the night's preferred narrow
substrate; $89.0\pm3.5$, highly variable across seeds, on ours; paired margins $+2.5$
$[+1.3,+4.8]$ and $+2.6$ $[+0.2,+5.2]$), above unmasked interleaved ER at the same budget
($87.5\pm0.8$ under Adam, $85.4\pm3.5$ under SGD) and above BP+ER ($88.8\pm0.3$; $+2.8$
$[+2.5,+3.2]$). In a single pass over the stream (one epoch per task, three seeds) the
ordering sharpens: $91.8\pm0.2$ against $88.7\pm0.3$ for BP+ER, $76.0\pm20.1$ for unmasked ER
and $75.1\pm3.5$ for the offline night, for which one night per task is too few
...
this buffer size. At very small buffers ($K{=}200$) the night retains an edge only in
combination with narrow substrates (Appendix~\ref{app:k200}).
...
the same-substrate night, bimodal on the first split, reaches $91.5\pm0.5$ on the second and
trails by only $0.3$ there (Appendix~\ref{app:val}).
...
only $78.0\pm3.2\%$, below even unmasked interleaved ER ($87.5\pm0.8$ under Adam) and far
...
thus costs fourteen points ($20$, with high seed variance, $70.3\pm10.1$, at the $5\%$
...
axes ($89.9\pm0.7$ sequential, $84.3\pm1.0$ static): all-or-none OFF periods beat turned-down
...
the underfit regime. At the $10\%$ fraction the MNIST price is $1.2$--$1.7$ points ($94.1$
vs.\ $95.3$--$95.8$ rotation-free) and the CIFAR static effect is inconclusive under the
available seeds ($28.0\pm1.2$ vs.\ $30.0\pm1.9$). Under masked Adam, a relative-novelty
...
local sleep $28.9\pm1.0$ $>$ offline night $25.7\pm1.2$ $>$ BP+ER $24.8\pm0.3$ $>$ unmasked
ER $15.4\pm4.2$ $\approx$ no buffer $16.3$ (the same on the second split:
$29.7>26.5>25.8>15.5$), despite task-active code overlap of $0.9$--$0.98$. On the V1-like
...
its learning rule. On raw pixels the local learner leads outright.
...
identical. Held-out split; headline rows carry six seeds, the rest three.}
\label{tab:ablation}
...
rotation's gain ($91.0\pm0.8$) but trails it by $0.6$ sequential and $2.0$ static points
with $1.4\times$ the forgetting ($F{=}8.7$ vs.\ $6.4$): alternating the coalition carries
...
$95.3$--$95.8$ without it, and $93.7$ with rotation and no isolation), a price of
$1.2$--$1.7$ points at the $10\%$ fraction. The unmasked control is not handicapped by the
...
six seeds). Blue: non-dominated by seed means (the bout-committed gate, the silent mask, unmasked
rotation-free replay); purple: dominated; held-out split. The top cluster (always-on rotation,
bout gate, anchor, masked Adam) lies within seed noise of one another; the frontier trades
sequential for static accuracy through the silent and unmasked variants, and the adaptive-$\lambda$
point (Section~\ref{sec:controller}) is dominated on this split.}
\label{fig:frontier}
\end{figure}
...
The construction is a recipe with two checkable prerequisites. If a system's representation
is sparse enough that a batch leaves most units silent, and its learning rule yields a local
error signal, consolidation can run in the silent degrees of freedom between the steps of
the stream, with no separate offline phase, under a conditional guarantee on the computation
in use, and at a higher total compute ($6\times$ the night's wall-clock, unoptimised). Both prerequisites can be checked on a given architecture. Top-$k$ routed mixture-of-experts
...
local error signal is what such a system would still have to supply.
...
Appendix~\ref{app:probes}). All results are at MLP scale on MNIST/CIFAR variants with three
seeds (six on the headline rows). BP+ER is an external reference rather than a
state-of-the-art claim, and stronger replay baselines such as DER++ are not included; it
...
itself. Rotation keeps a $1$--$2$-point static price at the $10\%$ fraction that no gate we
...
offline-night schedules on both held-out splits (above the best night by $2.5$ on the first
and by $0.3$ on the second) and consistently above BP+ER and unmasked local replay, at
$2.2\times$ the night's replay samples (and, on the first split, at $1.1\times$); it pays
one to two points on i.i.d.\ streams, gains outright on harder data, and runs at a twentieth
of dense-rate inference energy when spiking. The offline night retains one irreducible role,
consolidating very small episodic stores, which sharpens rather than blurs the division of
labour between wake and sleep.
