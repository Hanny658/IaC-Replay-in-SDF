...
and five-hidden-layer stacks that defeat every fixed decay. We provide exactness
...
$96.1\pm0.2$ is the best i.i.d.\ accuracy the substrate has produced under \emph{any}
configuration, isolation-free included. On CIFAR-100 the controller lifts the system
...
\paragraph{Depth is the same boundary, and the controller crosses it.}
...
$3\times10^{-6}$ in the starved middle layers. A BP+ER reference at the same widths is
depth-insensitive ($\approx\!88$--$89$ sequential throughout) but forgets $2$--$3\times$ more
($F\approx13$--$14$ vs.\ $5$--$8$); the local system under the controller matches or exceeds
it at every depth tested (three seeds).
...
(iv) convolutional stacks and residual-style skip routing --- the depth question itself is
now answered (Section~\ref{sec:controller}: five hidden layers at graceful cost under the
controller, collapse under every fixed decay), the routing is not;
