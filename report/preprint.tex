...
machinery; rotation carries most of the gain (rotation with unmasked replay reaches
$92.1\pm0.5\%$; isolation adds $0.6$ points and the guarantee, $1.8$ without rotation); the
headline orderings replicate on a held-out split never used for selection ($91.6\pm0.3\%$
against the night's $89.0\pm3.5\%$). Rotation's i.i.d.\ price is governed by the substrate's activity fraction: at the $5\%$
...
dataset-independent ratio target in place of per-dataset tuning --- trades $0.5$--$3$ sequential points for the best static accuracies we measure and lifts the system onto 100-class streams
...
(Appendix~\ref{app:val}, where the CIFAR-10 orderings replicate as well). Rotation alone (unmasked replay) reaches $91.5\pm0.3$ on the same split, 0.1 below
the full system, and $93.7\pm0.3$ static against $94.1\pm0.2$: the sub-additivity of
Table~\ref{tab:ablation} replicates. Every other number in this
...
$+3.9$ on top of isolation), and isolation's marginal accuracy is $0.6$ points at
batches $8$--$16$, within seed noise, with no static-side difference ($94.7$ either way).
What isolation buys at this configuration is the guarantee that, for the current input under its suppression
...
the outcome rests on the rotation's tolerance of that perturbation. The
...
trails it by $0.6$ at $8$--$16$ and holds at $4$, where the masked system turns
...
\subsection{The direction of protection matters, and the least optimiser state wins}
\label{sec:direction}
...
Section~\ref{sec:ablation} traces masked Adam's deficit to the confinement of its moments, which the confinement of the
update over time demands but accuracy, it turns out, does not. Third, \emph{soft}
...
with a $3.7$-fold smaller seed deviation ($92.7\pm0.3$ vs.\ $87.0\pm1.1$), isolation alone $+1.8$ ($88.8$ vs.\ $87.0$), isolation on top of rotation $+0.6$ ($92.1\pm0.5$ for
rotation with unmasked replay), and replay is worth everything ($19.6$ is the forgetting floor). Replay confined to the readout row, hidden
layers frozen for replay, falls to $27.3\pm15.1$ (ten classes) and costs $12$ static points: readout-only
replay cannot account for the full system's accuracy, and hidden-layer replay updates are
essential to it. A random suppression of matched size in place
...
default and the rotation-free unmasked control never closes the gap; the sweep bounds that
control, it does not estimate isolation's own contribution (the square above does). The gates that were built to refund a larger price at the
...
Table~\ref{tab:controller} gives the cross-dataset verdict, with a consistent signature. On the sequential axis the controller concedes $0.5$--$1.4$ points to the per-dataset tuned
value in its gradient-referenced form and $1.0$--$3.0$ in the drive form; on every static axis it wins, by $+0.5$ to $+3.6$ --- the fixed decay had
...
where most of the accuracy comes from: with it, even unmasked replay holds $92.1$ at batch $16$
and $89.5$ at batch $4$. Confining the update to the mask adds $0.6$ points at batches $8$--$16$
and the guarantee that the present input's hidden computation, under its suppression mask, is
untouched --- exactly on the proven channels, and for all but $\approx0.3\%$ of waking samples
per step in measurement (Propositions~\ref{prop:pre}--\ref{prop:post} and the Remark) --- while
at batch $4$ the masked system itself turns bimodal. The offline night enjoys neither: with
...
stream is using. Its accuracy value is $1.8$ points without rotation and $0.6$ with it
(Table~\ref{tab:ablation}): once the coalition alternates, the live units tolerate
unmasked replay, and what isolation adds is the guarantee that they need not. With isolated replay, refractory
