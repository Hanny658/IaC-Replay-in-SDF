...
full: exact isolation $+$ rotation $+$ progress gate & $\mathbf{89.1\pm0.3}$ & $\mathbf{95.0\pm0.6}$ \\
$-$ gate (rotation always on) & $92.0\pm0.7$ & $93.4\pm0.2$ \\
$-$ rotation (pre-silent isolation only) & $89.0\pm0.5$ & $96.0\pm0.1$ \\
...
the price in full ($95.0$, at the $95.1$ ceiling). The gate, however, is not free under SGD:
gated rotation loses $2.9$ sequential points against always-on ($89.1$ vs.\ $92.0$) --- a
gated system behaves almost exactly like the rotation-free one on the sequential axis
($89.1$ vs.\ $89.0$) --- where the same gate under Adam costs only $0.4$ ($90.4$ vs.\
$90.8$). The stateless gain requires rotation to run on every batch; with SGD one therefore
chooses between the sequential operating point ($92.0/93.4$, always-on) and the static one
($89.1/95.0$, gated), a genuine trade that the weaker Adam system does not face.
...
gate keeps a $0.7$-point static residue under Adam and costs $2.9$ sequential points under the
stateless optimiser (Section~\ref{sec:ablation}); and the novelty claims rest on our own literature
...
itself); (ii) reconciling the progress gate with the stateless optimiser, whose gain requires
rotation on every batch (Section~\ref{sec:ablation});
