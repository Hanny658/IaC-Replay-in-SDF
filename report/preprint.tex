...
(and reaching $91.8\pm0.3\%$ at a matched $1.2\times$ budget with eight-sample
...
homeostatic scaling but is fixed at $1$ throughout. Errors are produced by one top-down sweep: with target
$y$,
\begin{equation}
...
data gradient $G$ and decay $\lambda$, a replay step is
\begin{equation}
...
below concern the hidden computation.
...
cannot promote the unit), the hidden activities on $X$ are unchanged.
\end{proposition}
\begin{proof}
...
it is still excluded by $\Phi_k$ (or rectified to zero), so $a^\ell_{i,b}=0$ still, all other
units' inputs are untouched by Proposition~\ref{prop:pre}'s argument, and the layer's
...
$18{,}760\times16\approx3.0\times10^5$ samples ($2.4\times$ the night); eight-sample
micro-batches cost $1.5\times10^5$ ($1.2\times$) for $91.8\pm0.3$; pressure-triggered bursts
with batch $64$ cost $1.8\times10^5$ ($1.4\times$) for $89.1\pm1.1$.
...
is untouched. The decay removes a fixed fraction of what learning currently adds: layers and
regimes with rich supervision keep a strong regulariser, starved ones are automatically
spared. $\rho$ is the only control target; $\lambda_{\max}$, the EMA coefficient and
...
project. The bypass alone also rescues the fixed decay ($91.3/93.6$ at depth five), consistent with
error-path attenuation, rather than decay mis-tuning per se, being a principal contributor to
the depth collapse; the controller still adds its usual static margin on top of the bypass. A BP+ER reference at
...
$2.4\times$ the night's replay samples (within a point of it at a matched budget), pays about
