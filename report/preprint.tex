...
present one, the hidden computation on the current input is held invariant (exactly for
silent and suppressed units, and for all but $0.3\%$ of waking samples per update elsewhere)
...
(Figure~\ref{fig:frontier}) the controller point sits on the static side; the tuned fixed
decay remains the best sequential-axis setting.
...
in use, and at a higher total compute ($6\times$ the night's wall-clock, unoptimised). Top-$k$
routed mixture-of-experts layers already expose, per token, the experts a batch leaves
silent, so the sparse-support condition holds there by construction; the local error signal
is what such a system would still have to supply.
...
masked system itself destabilises at replay gain $10$ (Appendix~\ref{app:probes}). The
