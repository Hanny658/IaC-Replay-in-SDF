...
offline nights degrade sharply at the same sizes. The i.i.d.\ cost of rotation is removed either by gating rotation on relative novelty (under
masked Adam) or, under the stateless optimiser, by committing the gate to sleep-bout-like
episodes ($92.0/94.2$ vs.\ $92.0/93.3$ over six seeds); relaxing the substrate's inherited
$5\%$ activity fraction to $10$--$15\%$ improves both axes at once ($92.7/94.7$) and shrinks
the price at source. On split CIFAR-10 the ordering of methods transfers, the rotation price
reverses into a gain, and a substrate decomposition shows that the remaining gap to
backpropagation-with-replay is dense activation, not credit assignment. We provide exactness
...
once consolidation runs during wake, the offline phase is redundant at this buffer size. At very
...
Always-on rotation costs $1.3$--$3.0$ points on i.i.d.\ MNIST; on split CIFAR-10 the price
\emph{reverses} into a $+2.5$ to $+6.5$-point static gain --- forced rotation is a regulariser
in the underfit regime. Under masked Adam, a relative-novelty/progress gate
(Eqs.~\eqref{eq:nov}--\eqref{eq:prog}) removes the price with one setting across all four
regimes; the full Adam-era gate study --- including why absolute thresholds provably cannot
serve and why unit-level pressure gating only trades the axes --- is in
Appendix~\ref{app:gates}. Under the stateless optimiser the gate must instead be committed to
bouts (Section~\ref{sec:ablation}), and the sparsity dial (Section~\ref{sec:dial}) shrinks
the price at source.
...
fewer synaptic operations per pass --- is reproduced in Appendix~\ref{app:substrate}. Every mechanism cell shares the same
...
always-on rotation as an operating point. The static ceiling of silent isolation ($96.0$) stays out of reach --- rotation, even in
bouts, keeps a residual price, which bout-length and OFF-refractory controls locate in the
...
One budget note completes the block: the stateless gain vanishes at half cadence
($91.3\pm0.3$ with replay batch $64$ every other batch, identical to masked Adam) ---
statelessness pays only while consolidation runs continuously.
...
rotation-free) in the rotation of settled coalitions itself.
Two mechanisms attack that diagnosis directly (details in Appendix~\ref{app:probes}).
...
\emph{sequential stabiliser}: $92.4\pm0.2$ --- the best sequential cell in
Table~\ref{tab:ablation}, with the seed variance halved --- at static parity. Anchor and bout
gate compose sub-additively. Figure~\ref{fig:frontier} draws the resulting operating frontier.
...
Table~\ref{tab:ablation} and Appendix~\ref{app:substrate} --- every mechanism comparison
above is internally matched, and the orderings we spot-checked at $10$--$15\%$ are preserved
--- but
