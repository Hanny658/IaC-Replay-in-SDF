# MLP-Cortex：一个尽可能贴近人脑工作方式的神经网络（v1–v9 实验全记录）

*2026-08-27 至 2026-08-31 · 与课程作业共用代码库但完全独立（`report/` 与报告用 `results/` 未动）*
...
5. 皮层约束几乎免费、能耗 18× 改善（v5/v6：硬三件套 + k-WTA + T=0 + 30% 距离连接 + T_s=8 脉冲）。
...
- 只在 MNIST / split-MNIST / 三个表格数据集尺度验证；未测 CIFAR 级。
- K=200（小缓冲）局部睡眠仍逊夜间 3–5 pt；夜间在小缓冲上仍是更好的巩固器。
- 静态轴 −1.3~−3.0 的轮休代价未消除（候选：压力自适应的轮休比例）。
- 能耗叙事需更新：重放样本量 ≈ 清醒样本量的 0.5–1×，synops 未变；脉冲域未重测 v9。
...
关键配置名：`g9a_w512s5_cad2_br64`（v9 最优）、`g9a_w512s5_br8`（最省样本）、
